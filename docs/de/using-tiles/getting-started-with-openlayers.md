...
[OpenLayers](http://openlayers.org/){: target=blank} ist eine vollständige JavaScript Library zur Einbindung von Karten. s verwendet eine freizügige BSD Open-Source Lizenz - kann also ohne Bedenken legal in jede Seite integriert werden. Der Quellcode ist auf [GitHub](https://github.com/openlayers/ol3/){: target=blank} verfügbar.

Wir beschränken uns hier auf ein kleines, eigenständiges Beispiel und verweisen auf die offiziellen [Tutorials](http://openlayers.org/en/latest/examples/) und [API](http://openlayers.org/en/latest/apidoc/) für komplexere Anwendungen.
...
* alle Standorte Ihres Unternehmens hinzufügen? → s Sie sie als [GeoJSON](http://geojson.org/){: target=blank} bereit und [integrieren](http://openlayers.org/en/latest/examples/select-features.html){: target=blank} Sie sie in die Karte.
